...
SELECT billing_country, count(total) AS highest_invoices FROM invoice 
...
SELECT city, SUM(total) AS total_invoice FROM customer JOIN invoice ON  invoice.customer_id = customer.customer_id 
GROUP BY city 
